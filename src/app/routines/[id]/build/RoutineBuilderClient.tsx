...
import { saveRoutineConfiguration } from "@/app/actions/routines";
import { Exercise, RoutineExercise, RoutineWithExercises, useAllExercises, useRoutineById } from "@/hooks/queries/useRoutineById";
...
/**
 * RoutineBuilderClient - Client Component for building routines
 * 
 * Uses React Query for cached navigation.
...
export function RoutineBuilderClient({ 
    ...
    // React Query for routine data - enables cached navigation
    const { data: routine, refetch } = useRoutineById(initialRoutine.id, initialRoutine);
    
    // React Query for exercises - cached across app
    const { data: availableExercises = initialExercises } = useAllExercises(initialExercises);
    ...
    // Sync local exercises when routine data changes (e.g., after save)
    useEffect(() => {
    ...
    const handleSave = async () => {
        if (!isDirty && localExercises.length > 0) return;
        if (!routine) return;
        ...
        try {
            ...
            await refetch(); // Refresh cache with new IDs
            setIsDirty(false);
        } catch (error) {
